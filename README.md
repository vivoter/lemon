...
#### 4.1.2.1 Component
在类上添加Component注解，相当于配置文件中的<bean id="" class=""/>。Component 有个name属性，配置name属性表示在BeanFactory cacheMap里面存此对象的key是配置的name，若默认不配置则使用该类的首字母小写为关键字
@Component
public class Demo1Model {}

#### 4.1.2.2 Autowired
使用在属性上，自动装配属性，默认使用属性的名字
@Autowired
public Context mContext;

#### 4.1.2.3 RefBean
使用在属性上，有个name属性，根据名称装配
@RefBean(name = "lemonMessage")
public LemonMessage lemonMessage;

#### 4.1.2.4 InitMethod
使用在方法上，配置了InitMethod方法，在app启动的时候会被执行
@InitMethod
public void init() {
    name = "demo1  model";
}
